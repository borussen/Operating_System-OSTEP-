...
void find_dir(char *pathname, int currentDepth, int maxDepth, regex_t *preg) {
    ...
    while ((d = readdir(dp)) != NULL) {
        ...
        if (strncmp(d->d_name, ".", 1) != 0 && strncmp(d->d_name, "..", 2) != 0) {
            if (preg == NULL || regexec(preg, d->d_name, 0, NULL, 0) != REG_NOMATCH) { printf("%s\n", filePath); }
            if (d->d_type == DT_DIR) { find_dir(filePath, currentDepth, maxDepth, preg); }
        }
    }
    closedir(dp);
}
...
int main(int argc, char **argv) {
    ...
    if (enable_pattern && regcomp(&preg, pattern, 0) != 0) { handle_error("regcomp"); }

    if (stat(pathname, &sb) == -1) { handle_error("stat"); }

    if (!enable_pattern || (enable_pattern && regexec(&preg, pathname, 0, NULL, 0) != REG_NOMATCH)) { printf("%s\n", pathname); }

    if (S_ISDIR(sb.st_mode)) {
        if (enable_pattern) { find_dir(pathname, currentDepth, maxDepth, &preg);}
        
        else { find_dir(pathname, currentDepth, maxDepth, NULL); }
    }
